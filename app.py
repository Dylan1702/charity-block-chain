...
# Function to generate a signature for a given message using a secret key
def generate_signature(message, secret_key):
    hash_object = hashlib.sha256(message.encode('utf-8'))
    hmac_object = hmac.new(secret_key.encode('utf-8'), hash_object.digest(), hashlib.sha256)
    signature = hmac_object.hexdigest()
    return signature

# Function to verify the signature of a given message using a secret key
def verify_signature(message, signature, secret_key):
    expected_signature = generate_signature(message, secret_key)
    return hmac.compare_digest(signature, expected_signature)
import mysql.connector
db=mysql.connector.connect(user='root',port=3307,database='charity')
cur=db.cursor()
...
def update_data():
    ...
    updated_hash = hashlib.sha256(data.encode()).hexdigest()

    # Check if the hashes match
    if original_hash != updated_hash:
        # If the hashes don't match, display a tampering message
        return render_template('tampering_message.html')
    else:
        # If the hashes match, display a success message
